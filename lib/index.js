...
class TicketFinder {
  ...
  _parseTickets(html, date, route) {
    ...
    return items.map((item) => {
      let $item = $(item);
      let departure = /Salida\s+(.*)\s+/.exec($item.find('.two > p:nth-child(1)').text());
      let arrival = /Llegada\s+(.*)\s+/.exec($item.find('.two > p:nth-child(2)').text());
      let price = /([\d.]+)/.exec($item.find('.five > p').text());
      return new Ticket({
        origin: route.origin,
        destination: route.destination,
        date: date,
        departure: (departure && departure[1]).trim(),
        arrival: (arrival && arrival[1]).trim(),
        price: price && +price[1]
      });
    });
  }
